...
ZkClient::~ZkClient() {
  if (m_zhandle != nullptr) {
    zookeeper_close(m_zhandle);  // 类似，关闭句柄，释放资源  MySQL_Conn
  }
}
...
void ZkClient::Start() {
  ...
  API调用线程:当前start线程
  同时，启动了网络I/O线程  pthread_create  poll  不需要高并发
  watcher回调线程 pthread_create
  zookeeper_init起来之后，调用网络i/o线程和watcher线程
  */
  // 3000 会话超出时间
  ...
  sem_t sem;                         // 设置信号量
  sem_init(&sem, 0, 0);              // 资源初始化为0
  zoo_set_context(m_zhandle, &sem);  // 给watcher传参数，设置上下文

  sem_wait(&sem);  // 初始化为0，主线程等待，下不来
